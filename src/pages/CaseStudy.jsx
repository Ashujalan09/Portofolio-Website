...
export default function CaseStudyPage() {
  ...
  if (!c) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: T.cream, fontFamily: "'DM Sans', sans-serif" }}>
        <p style={{ color: T.muted, fontSize: 18, marginBottom: 24 }}>Case study not found.</p>
        <Link to="/" style={{ color: T.clay, fontWeight: 500 }}>← Back to home</Link>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: T.cream, minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ── Top Nav ── */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 60px", background: "rgba(253,246,238,0.92)", backdropFilter: "blur(14px)", borderBottom: "1px solid rgba(212,98,58,0.1)" }}>
      ...
      {/* ── Hero ── */}
      <section style={{ paddingTop: 140, paddingBottom: 80, paddingLeft: 60, paddingRight: 60, background: c.grad, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 860 }}>
          ...
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(36px,5vw,72px)", fontWeight: 500, letterSpacing: -2, color: T.ink, lineHeight: 1.1, marginBottom: 20 }}>{c.title}</h1>
          <p style={{ fontSize: 18, color: T.muted, maxWidth: 600, lineHeight: 1.7, fontWeight: 300 }}>{c.subtitle}</p>

          {/* Meta row */}
          <div style={{ display: "flex", gap: 40, marginTop: 48, flexWrap: "wrap" }}>
            {[["Role", c.role], ["Timeline", c.timeline], ["Tools", c.tools]].map(([label, val]) => (
              <div key={label}>
      ...
      {/* ── Problem ── */}
      <section style={{ padding: "80px 60px", background: T.warmWhite }}>
        <div style={{ maxWidth: 760 }}>
          <SectionLabel>The Problem</SectionLabel>
          <p style={{ fontSize: 18, color: T.ink, lineHeight: 1.8, fontWeight: 300 }}>{c.problem}</p>
        </div>
      </section>

      {/* ── Process ── */}
      <section style={{ padding: "80px 60px", background: T.cream }}>
        <SectionLabel>The Process</SectionLabel>
        <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(28px,3vw,44px)", fontWeight: 500, letterSpacing: -1, color: T.ink, marginBottom: 48, lineHeight: 1.2 }}>
          How I approached it
        </h2>
      ...
      {/* ── Outcome ── */}
      <section style={{ padding: "80px 60px", background: T.ink, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 760 }}>
          ...
          <p style={{ fontSize: 18, color: "rgba(255,255,255,0.85)", lineHeight: 1.8, fontWeight: 300 }}>{c.outcome}</p>
        </div>
      </section>

      {/* ── Next Project ── */}
      <section style={{ padding: "80px 60px", background: T.cream, textAlign: "center" }}>
        <p style={{ fontSize: 12, letterSpacing: 2.5, textTransform: "uppercase", color: T.clay, fontWeight: 500, marginBottom: 16 }}>Next Project</p>
        <Link to={`/case-study/${next.slug}`} style={{ textDecoration: "none" }}>
          <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(24px,3vw,40px)", fontWeight: 500, color: T.ink, letterSpacing: -1, marginBottom: 24, lineHeight: 1.2 }}>{next.title}</h3>
