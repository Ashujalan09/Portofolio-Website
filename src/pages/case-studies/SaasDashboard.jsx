...
export default function SaasDashboard() {
  ...
  return (
    <div style={{ background: T.cream, minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ── Hero ── */}
      <section style={{ paddingTop: 140, paddingBottom: 80, paddingLeft: "clamp(20px,6vw,60px)", paddingRight: "clamp(20px,6vw,60px)", background: "linear-gradient(135deg,#e8d8f0,#c4a8d8)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 860 }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
            {[{ label: "Design Systems", text: "#7a4aa0" }, { label: "Web App", text: "#7a4aa0" }].map((t) => (
          ...
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(36px,5vw,72px)", fontWeight: 500, letterSpacing: -2, color: T.ink, lineHeight: 1.1, marginBottom: 20 }}>SaaS Dashboard</h1>
          <p style={{ fontSize: 18, color: T.muted, maxWidth: 600, lineHeight: 1.7, fontWeight: 300 }}>A unified command centre for platform administrators</p>
          <div style={{ display: "flex", gap: 40, marginTop: 48, flexWrap: "wrap" }}>
            {[["Role", "Lead UX Designer"], ["Timeline", "5 months"], ["Tools", "Figma, Storybook, Zeroheight"]].map(([label, val]) => (
              <div key={label}>
      ...
      <section style={{ padding: "clamp(40px,7vw,80px) clamp(20px,6vw,60px)", background: T.warmWhite }}>
        <div style={{ maxWidth: 760 }}>
          ...
          <p style={{ fontSize: 18, color: T.ink, lineHeight: 1.8, fontWeight: 300 }}>Platform admins were juggling 4 separate tools to manage users, billing, analytics, and support. Context switching was constant, critical alerts were missed, and onboarding new admins took weeks.</p>
        </div>
      </section>
      ...
      <section style={{ padding: "clamp(40px,7vw,80px) clamp(20px,6vw,60px)", background: T.ink, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 760 }}>
          ...
          <p style={{ fontSize: 18, color: "rgba(255,255,255,0.85)", lineHeight: 1.8, fontWeight: 300 }}>Consolidated 4 tools into one. Admin onboarding time dropped from 3 weeks to 4 days. Task completion rate on critical actions improved by 55%. Design system adopted across 3 product teams.</p>
        </div>
      </section>
      ...
      <section style={{ padding: "clamp(40px,7vw,80px) clamp(20px,6vw,60px)", background: T.cream, textAlign: "center" }}>
        ...
        <Link to="/case-study/national-pension-scheme" style={{ textDecoration: "none" }}>
          <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(24px,3vw,40px)", fontWeight: 500, color: T.ink, letterSpacing: -1, marginBottom: 24, lineHeight: 1.2 }}>Nation Pension Scheme Onboarding</h3>
